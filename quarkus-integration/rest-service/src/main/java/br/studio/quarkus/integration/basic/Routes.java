...
public class Routes extends RouteBuilder {
    ...
    private final Set<Fruit> fruits = Collections.synchronizedSet(new LinkedHashSet<>()); //Uses thread-safe collections to store fruits and legumes


    private final Set<Legume> legumes = Collections.synchronizedSet(new LinkedHashSet<>()); //Uses thread-safe collections to store fruits and legumes
    ...
    public void configure() throws Exception {
        ...
        from("direct:addFruit")//endpoint addFruit
                .to("log:br.studio.quarkus.integration.basic?level=INFO") //loga a requisição
                .process().body(Fruit.class, fruits::add) //adiciona a fruta recebida na lista de frutas
                .setBody().constant(fruits);//retorna a lista de frutas
